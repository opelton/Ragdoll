...
namespace Potato.FPS.Gameplay
{
    public class ProjectileStandard : ProjectileBase
    {
        ...
        void Start()
        {
            m_HitLayers = LayerHelper.GetCollisionMaskForLayer(gameObject.layer);
        }
        ...
        new void OnShoot()
        {
            ...
            if (playerWeaponsManager)
            {
                ...
                if (Physics.Raycast(playerWeaponsManager.WeaponCamera.transform.position, cameraToMuzzle.normalized,
                    out RaycastHit hit, cameraToMuzzle.magnitude, m_HitLayers, k_TriggerInteraction))
                {
                    if (IsHitValid(hit))
                    {
        ...
        void Update()
        {
            ...
            // Hit detection
            {
                ...
                RaycastHit[] hits = Physics.SphereCastAll(m_LastRootPosition, Radius,
                    displacementSinceLastFrame.normalized, displacementSinceLastFrame.magnitude, m_HitLayers,
        ...
        void OnHit(Vector3 point, Vector3 normal, Collider collider)
        {
            ...
            {
                // area damage
                AreaOfDamage.InflictDamageInArea(Damage, point, m_HitLayers, k_TriggerInteraction,
